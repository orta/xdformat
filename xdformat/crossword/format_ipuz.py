# -*- coding: utf-8 -*-
from .core import Crossword, CrosswordCell
from .exceptions import CrosswordException


# ipuz clue entries: [number, clue] pairs (older ipuz lib) or dicts (newer)
def _clue_entry(entry):
...
def from_ipuz(ipuz_dict):
    ...
    for entry in ipuz_dict.get('clues', {}).get('Across', []):
        number, clue = _clue_entry(entry)
        crossword.clues.across[number] = clue
    for entry in ipuz_dict.get('clues', {}).get('Down', []):
        number, clue = _clue_entry(entry)
